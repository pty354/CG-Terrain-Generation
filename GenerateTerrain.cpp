...
Terrain::~Terrain()
{
	delete diamondSquareAlg, vor;
}
void Terrain::init() {
	//get Voronoi map and noise map
	diamondSquareAlg = new DiamondSquareAlg(width, initialSpread);
	vor = new Voronoi(iterationTimes, randomPointsNum, -1.0f, 1.0f, width); 
	diamondSquareAlg = MidpointDisplacement(diamondSquareAlg, iterationTimes);
	N = vor->N;
	//The map we get from midpoint displacement is disordered
	for (int i = 0; i < N * N; i++)
		diamondSquareAlg->v.push_back(diamondSquareAlg->points[i]->p);
	sort(diamondSquareAlg->v.begin(), diamondSquareAlg->v.end(), comp);

}
void Terrain::update(float ratio) {
	v.clear();
	//Addition of noise and Voronoi map
	for (int i = 0; i < N * N; i++) {
		v.push_back(diamondSquareAlg->v[i]);
		v[i].y = diamondSquareAlg->v[i].y * ratio + vor->v[i].y * (1.f - ratio);
